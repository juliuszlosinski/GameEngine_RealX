#ifndef PHYSX_DYNAMIC_CAPSULE3D_SYSTEM
#define PHYSX_DYNAMIC_CAPSULE3D_SYSTEM
...
class PhysicsDynamicCapsule3DSystem : public System
{
public:
	void Update(EntityComponentSystem& g_Coordinator, PhysicsEngine* pPhysX)
	{
		for (auto const& entity : m_Entities)
		{
			...
			auto& physXCapsule3D = g_Coordinator.GetComponent<PhysXDynamicCapsule3DComponent>(entity);

			if (physXCapsule3D.physXRigidBody == nullptr)
			{
				physXCapsule3D.physXRigidBody = pPhysX->CreateDynamicCapsule(
				...
				physXCapsule3D.physXRigidBody->setMass(physXCapsule3D.mass);
				physXCapsule3D.physXRigidBody->setName(tag.name);
			}

			if (physXCapsule3D.physXRigidBody != nullptr)
			{
				physx::PxTransform pxTransform = physXCapsule3D.physXRigidBody->getGlobalPose();
