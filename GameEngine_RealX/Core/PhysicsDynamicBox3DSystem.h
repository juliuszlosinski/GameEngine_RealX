#ifndef PHYSX_DYNAMIC_BOX3D_SYSTEM
#define PHYSX_DYNAMIC_BOX3D_SYSTEM
...
class PhysicsDynamicBox3DSystem : public System
{
public:
	void Update(EntityComponentSystem& g_Coordinator, PhysicsEngine* pPhysX)
	{
		for (auto const& entity : m_Entities)
		{
			...
			auto& physXBox3D = g_Coordinator.GetComponent<PhysXDynamicBox3DComponent>(entity);

			if (physXBox3D.physXRigidBody == nullptr)
			{
				physXBox3D.physXRigidBody = pPhysX->CreateDynamicBox(
				...
				physXBox3D.physXRigidBody->setMass(physXBox3D.mass);
				physXBox3D.physXRigidBody->setName(tag.name);
			}

			if (physXBox3D.physXRigidBody != nullptr)
			{
				physx::PxTransform pxTransform = physXBox3D.physXRigidBody->getGlobalPose();
